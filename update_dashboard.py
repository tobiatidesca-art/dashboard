...
# --- MODULO 2: STRATEGY_CORE (UPDATED WITH GRANULAR TIMESTAMPS) ---
def calculate_quant_logic(prices, opens):
    ...
    # Snapshot con Timestamp per singolo asset
...
def get_language_pack():
    return {
        "en": {
            "title": "QUANT-PRO V5.6", "sync": "MODULO 1: DATA_SYNC", "core": "MODULO 2: CORE_LOGIC",
        ...
        "de": {
            "title": "QUANT-PRO V5.6", "sync": "MODUL 1: DATEN_SYNC", "core": "MODUL 2: KERN_LOGIK",
        ...
        "fr": { "title": "QUANT-PRO V5.6", "sync": "MODULE 1: SYNC", "core": "MODULE 2: LOGIQUE", "controls": "MODULE 3: CONTROLES", "chart": "MODULE 4: GRAPHIQUE", "system": "MODULE 5: SYSTEME", "threshold": "SEUIL (%)", "profit": "PROFIT NET", "equity": "Equité (€)", "benchmark": "Indice", "select_asset": "ACTIF", "select_lang": "LANGUE", "market_info": "ÉTAT DU MARCHÉ", "entry": "ENTRÉE" },
        "es": { "title": "QUANT-PRO V5.6", "sync": "MÓDULO 1: SYNC", "core": "MÓDULO 2: LÓGICA", "controls": "MÓDULO 3: CONTROLES", "chart": "MÓDULO 4: GRÁFICO", "system": "MÓDULO 5: SISTEMA", "threshold": "UMBRAL (%)", "profit": "BENEFICIO NETO", "equity": "Equidad (€)", "benchmark": "Índice", "select_asset": "ACTIVO", "select_lang": "IDIOMA", "market_info": "ESTADO DEL MERCADO", "entry": "ENTRADA" },
        "zh": { "title": "QUANT-PRO V5.6", "sync": "模块 1: 同步", "core": "模块 2: 逻辑", "controls": "模块 3: 控制", "chart": "模块 4: 图表", "system": "模块 5: 系统", "threshold": "阈值 (%)", "profit": "净利润", "equity": "权益 (€)", "benchmark": "指数", "select_asset": "资产", "select_lang": "语言", "market_info": "全球市场状态", "entry": "入场" }
    }
...
def generate_visual_interface(history, live):
    ...
    <head>
        ...
        <style>
            ...
            .market-detail {{ font-size: 0.82rem; border-bottom: 1px solid #30363d; padding: 6px 0; display: flex; justify-content: space-between; align-items: center; }}
            .ts-label {{ font-size: 0.7rem; color: var(--mod); margin-left: 8px; }}
    ...
    <body class="p-3">
        <div class="nav-header shadow-lg mb-3 rounded">
            <div class="d-flex justify-content-between align-items-center">
                <div><span class="mod-label" id="ui-mod1">{lang_pack['en']['sync']}</span><br><h2 class="h4 m-0 text-white" id="ui-title">QUANT-PRO</h2></div>
                <div class="text-end"><span class="mod-label" id="ui-mod2">{lang_pack['en']['core']}</span><br><div id="signal-display" class="h2 fw-bold text-warning m-0">---</div></div>
            </div>
        </div>
        ...
        <div class="container-fluid">
            <div class="row g-2 mb-3">
                <div class="col-md-4">
                    <div class="top-control-box" style="border-top-color: var(--blue)">
                        ...
                        <div class="market-detail mt-1">
                            <span>S&P 500 <small class="ts-label">{live['ts_sp']}</small></span>
                            <span class="text-white fw-bold">{live['sp_price']:,.2f}</span>
                        </div>
                        <div class="market-detail">
                            <span>NIKKEI <small class="ts-label">{live['ts_jpn']}</small></span>
                            <span class="text-white fw-bold">{live['jpn_price']:,.2f}</span>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="top-control-box" style="border-top-color: var(--accent)">
                        ...
                        <select class="custom-select mt-1" id="asset-select" onchange="run()">
                            <option value="eu">EURO STOXX 50 🇪🇺</option><option value="dax">DAX 40 🇩🇪</option>
                        </select>
                ...
                <div class="col-md-4">
                    <div class="top-control-box" style="border-top-color: var(--gold)">
                        ...
                        <select class="custom-select mt-1" id="lang-switch" onchange="run()">
                            <option value="en">English 🇺🇸</option><option value="de">Deutsch 🇩🇪</option><option value="fr">Français 🇫🇷</option><option value="es">Español 🇪🇸</option><option value="zh">中文 🇨🇳</option>
                        </select>
            ...
            <div class="row g-3">
                <div class="col-lg-3"><div class="card-mod shadow-sm"><span class="tag-mod" id="ui-mod3-tag">{lang_pack['en']['controls']}</span><label class="small fw-bold text-secondary mb-2 d-block" id="ui-threshold-label">{lang_pack['en']['threshold']}</label><input type="number" id="thr" class="form-control mb-4" value="0.30" step="0.05" oninput="run()"><div id="kpi-box"></div></div></div>
                <div class="col-lg-9"><div class="card-mod shadow-sm"><span class="tag-mod" id="ui-mod4-tag">{lang_pack['en']['chart']}</span><div style="height: 400px;"><canvas id="main-chart"></canvas></div></div></div>
            </div>
            <div class="text-center mt-3"><small class="text-muted" id="ui-mod5">{lang_pack['en']['system']} | ACTIVE</small></div>
        </div>

        <script>
            const raw = {json.dumps(history)}; const lD = {json.dumps(live)}; const langPack = {json.dumps(lang_pack)}; let chart = null;
            function run() {{
                const lang = document.getElementById('lang-switch').value; const asset = document.getElementById('asset-select').value;
                const t = parseFloat(document.getElementById('thr').value) / 100; const tP = langPack[lang];
                document.getElementById('ui-mod1').innerText = tP.sync; document.getElementById('ui-mod2').innerText = tP.core;
                document.getElementById('ui-mod3-tag').innerText = tP.controls; document.getElementById('ui-mod4-tag').innerText = tP.chart;
                document.getElementById('ui-mod5').innerText = tP.system + " | " + lD.last_update;
                document.getElementById('ui-asset-label').innerText = tP.select_asset; document.getElementById('ui-lang-label').innerText = tP.select_lang;
                document.getElementById('ui-market-info').innerText = tP.market_info; document.getElementById('ui-entry-label').innerText = tP.entry;
                ...
                raw.forEach(r => {{
                    let s = 0; if (r.m > t && r.v < 25) s = 1; else if (r.m < -t && r.v < 32) s = -1;
                    let pnl = (asset === 'eu') ? r.p_eu : r.p_dax; let idx = (asset === 'eu') ? r.idx_eu : r.idx_dax;
                    cap += (s * pnl); curve.push(cap); days.push(r.d); benchmark.push(idx);
                }});
                ...
                let sig = "FLAT ⚪"; if (lD.mom > t && lD.vix < 25) sig = "LONG 🟢"; else if (lD.mom < -t && lD.vix < 32) sig = "SHORT 🔴";
                document.getElementById('signal-display').innerText = sig;
                if (chart) chart.destroy();
                chart = new Chart(document.getElementById('main-chart'), {{
                    type: 'line', data: {{ labels: days, datasets: [{{ label: tP.equity, data: curve, borderColor: '#238636', borderWidth: 2, pointRadius: 0, fill: true, yAxisID: 'y' }}, {{ label: asset.toUpperCase() + " Index", data: benchmark, borderColor: '#58a6ff', borderWidth: 1, pointRadius: 0, fill: false, yAxisID: 'y1' }}] }},
