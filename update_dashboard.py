...
def get_data():
    ...
    live_vals['VIX'] = live_vals['VIX'] # Per il JS
    ...
    df['VIX'] = prices['^VIX']
    df['PNL_REAL'] = ((df['EU_C'] - df['EU_O']) - 2.0) * 10
    df['MOM_TOTAL'] = (df['USA_R'] + df['JAP_R'] + df['FUT_R']) / 3
    df = df.dropna()
    
    history = []
    for dt, row in df.iterrows():
        history.append({'d': dt.strftime('%d/%m/%Y'), 'm': float(row['MOM_TOTAL']), 'v': float(row['VIX']), 'p': float(row['PNL_REAL'])})
...
<head>
    ...
    <style>
        ...
        .kpi-box {{ background: #0d1117; border: 1px solid #30363d; padding: 10px; text-align: center; border-radius: 8px; }}
        input {{ background: #fff !important; color: #000 !important; font-weight: bold; }}
    </style>
</head>
<body class="p-3">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="card-custom">
                <label>Soglia Momentum (%)</label>
                <input type="number" id="threshold" class="form-control mb-3" value="0.30" step="0.05" oninput="run()">
                <div class="row g-2" id="kpiGrid"></div>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card-custom" style="height: 500px;"><canvas id="equityChart"></canvas></div>
        </div>
    </div>
    <script>
        ...
        function run() {{
            ...
            let cap = 20000; let equity = []; let labels = [];
            raw.forEach(item => {{
                let s = 0;
                if (item.m > thr && item.v < 25) s = 1; else if (item.m < -thr && item.v < 32) s = -1;
                cap += (s * item.p);
                equity.push(cap); labels.push(item.d);
            }});
            document.getElementById('kpiGrid').innerHTML = `<h3>Profit: ${(cap-20000).toFixed(0)}€</h3>`;
            if (chart) chart.destroy();
            chart = new Chart(document.getElementById('equityChart'), {{
                type: 'line', data: {{ labels: labels, datasets: [{{ label: 'Equity', data: equity, borderColor: '#238636', pointRadius: 0 }}] }},
